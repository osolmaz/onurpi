...
describe("integrity and read-only loading", () => {
  ...
  it("reports invalid headers, versions, roots, pairs, and bounded scan omissions", async () => {
    ...
    const unboundedHeader = await fixture([
      { type: "session", version: 3, id: "x".repeat(50_000), timestamp, cwd: "/fixture" },
    ]);
    ...
    const boundedJson = await execute([unboundedHeader, "--format", "json"]);
    expect(Buffer.byteLength(boundedJson)).toBeLessThanOrEqual(TOTAL_OUTPUT_BYTES);
    expect(boundedJson).not.toContain("x".repeat(1_000));
  });
  ...
  it("counts integrity issues suppressed by the report limit", async () => {
    const path = await fixture([
      header("019fd7b1-8888-7888-8888-888888888888"),
      ...Array.from({ length: 250 }, () => user("duplicate", null, "request")),
    ]);
    const loaded = await loadSession(path);
    expect(loaded.integrity.issues).toHaveLength(200);
    expect(loaded.integrity.omittedIssues).toBeGreaterThan(0);
    expect(loaded.integrity.issues.at(-1)?.code).toBe("integrity_issues_omitted");
  });
...
describe("CLI discovery and rendering", () => {
  it("resolves a unique UUID prefix in cwd and rejects broad or ambiguous lookup", async () => {
    ...
    const creatingListSpy = vi.spyOn(SessionManager, "list");
    const listSpy = vi.spyOn(SessionManager, "listAll").mockResolvedValue([info]);
    ...
    listSpy.mockResolvedValue([
      sessionInfo(path, `019fd7b1-api_key=first-secret-${"x".repeat(5_000)}`),
      sessionInfo(otherPath, `019fd7b1-api_key=second-secret-${"y".repeat(5_000)}`),
    ]);
    const ambiguity = await resolveSessionPath("019fd7b1", "/fixture", false).catch(
      (error: unknown) => String(error),
    );
    expect(Buffer.byteLength(ambiguity)).toBeLessThan(1_024);
    expect(ambiguity).not.toContain("first-secret");
    expect(ambiguity).not.toContain("second-secret");
    expect(creatingListSpy).not.toHaveBeenCalled();
