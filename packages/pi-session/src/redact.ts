...
function boundedOmissions(omissions: readonly string[]): string[] {
  if (omissions.length <= MAX_OMISSION_NOTICES) return [...omissions];
  return [
    ...omissions.slice(0, MAX_OMISSION_NOTICES),
    `${String(omissions.length - MAX_OMISSION_NOTICES)} more omission notices`,
  ];
}
