...
export async function listSessions(cwd: string, allProjects: boolean): Promise<SessionInfo[]> {
  return allProjects ? await SessionManager.listAll() : await SessionManager.list(cwd);
}
...
export async function loadSession(path: string): Promise<LoadedSession> {
  ...
  const issues = limitIssues([...initialIssues, ...headerIssues, ...validation.issues]);
...
function invalidLoadedSession(
  ...
  return {
    ...
    entries: [],
    branch: [],
    integrity: report(limitIssues(issues)),
  };
}
