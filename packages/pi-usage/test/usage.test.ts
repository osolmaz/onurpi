...
test("/usage automatically queries the current runtime account and shows state plus next actions", async () => {
...
test("explicit all-provider query labels current/configured and retains provider failures", async () => {
  const originalFetch = globalThis.fetch;
  onTestFinished(() => {
    globalThis.fetch = originalFetch;
  });
...
test("another-provider queries show only the selected provider and preserve current status", async () => {
  const originalFetch = globalThis.fetch;
  onTestFinished(() => {
    globalThis.fetch = originalFetch;
  });
...
test("current auth appearance during a command is revalidated before display", async () => {
  const originalFetch = globalThis.fetch;
  onTestFinished(() => {
    globalThis.fetch = originalFetch;
  });
...
test("TUI usage queries complete through the loader before opening the menu", async () => {
  const originalFetch = globalThis.fetch;
  onTestFinished(() => {
    globalThis.fetch = originalFetch;
  });
...
test("session shutdown aborts usage action and provider selectors", async () => {
  const originalFetch = globalThis.fetch;
  onTestFinished(() => {
    globalThis.fetch = originalFetch;
  });
...
test("automatic provider failures back off instead of retrying every turn", async () => {
  const originalFetch = globalThis.fetch;
  onTestFinished(() => {
    globalThis.fetch = originalFetch;
  });
...
test("a current command supersedes an older automatic query for the same provider", async () => {
  const originalFetch = globalThis.fetch;
  onTestFinished(() => {
    globalThis.fetch = originalFetch;
  });
...
test("cross-provider results revalidate which account is Current before display", async () => {
  const originalFetch = globalThis.fetch;
  onTestFinished(() => {
    globalThis.fetch = originalFetch;
  });
...
test("session shutdown clears status through the shutdown context", async () => {
  const originalFetch = globalThis.fetch;
  onTestFinished(() => {
    globalThis.fetch = originalFetch;
  });
...
test("a slow command cannot overwrite status after the selected model changes", async () => {
  const originalFetch = globalThis.fetch;
  onTestFinished(() => {
    globalThis.fetch = originalFetch;
  });
...
test("statusline follows runtime auth changes and clears for unsupported selected providers", async () => {
  const originalFetch = globalThis.fetch;
  onTestFinished(() => {
    globalThis.fetch = originalFetch;
  });
