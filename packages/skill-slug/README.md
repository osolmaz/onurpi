...
Skill Slug invokes a Pi skill when you type its bare slug and press enter. Typing `bro` is
equivalent to typing `/skill:bro`.
...
- The whole trimmed input must equal the slug exactly. `bro` rewrites; `bro please rewrite this`
  does not. Arguments remain available as `/skill:bro <args>`, which keeps natural-language
