...
- The built-in `openai-codex` provider and `openai-codex-*` switcher profiles are owned by
  `@onurpi/pi-codex-compaction` (native remote compaction); never arm the SSE retry override for
  them.
