...
`/retry-status` reports the current attempt and remaining delay. Retry indicators show the current
attempt without a finite total, such as `Retrying (3)` and `retry 3 in 8s`.
...
required private methods and retry indicator retain their expected signatures and remain patchable.
The extension fails closed on older versions or a changed runtime shape instead of applying a
partial patch.
