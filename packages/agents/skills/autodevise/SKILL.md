...
Use the built-in `autodevise` Pi Workflow when it is available. At top level, list workflows, then start `autodevise` once with the problem, authorized scope, constraints, previous plan, and new evidence from the conversation.
