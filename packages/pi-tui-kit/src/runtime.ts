...
// eslint-disable-next-line complexity, max-lines-per-function -- Preserve the audited upstream routine as a single unit; reviewed during vendoring.
...
async function showTuiScreen<
  ScreenId extends string,
  ActionId extends string,
  // eslint-disable-next-line @typescript-eslint/no-unnecessary-type-parameters -- Context threads the caller's context type into the callbacks.
  Context extends MenuContext,
>(
  ctx: Context,
  screen: MenuScreen<ScreenId, ActionId>,
    ...
    return await uiFor(ctx).custom<InternalScreenEvent<ScreenId> | undefined>(
      (tui, theme, keybindings, done) => {
        ...
        component = createMenuScreenComponent({
          ...
          keybindings,
          onEvent: finish,
          // eslint-disable-next-line @typescript-eslint/unbound-method -- The callback object owns no this state.
