...
export function createTuiHarness(options: TuiHarnessOptions = {}): TuiHarness {
  ...
  // eslint-disable-next-line @typescript-eslint/no-unnecessary-type-parameters -- Keeps the resolved value type at the call site.
  ...
  return {
    ...
    waitForOpen() {
      ...
      if (session?.settled && !session.opened && session.openError !== undefined) {
        // eslint-disable-next-line @typescript-eslint/prefer-promise-reject-errors -- The captured open error is already an Error or a deliberate rejection value.
