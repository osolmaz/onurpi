...
Switching between compact and expanded mode MUST update the existing transcript immediately. Expansion MAY replace the current session runtime to restore entries omitted by compact projection. Compaction MUST fold the currently loaded components in place and MUST NOT replace the session runtime, because Pi binds extension transcript projection after replacement replay. An explicit compact command MUST refresh loaded components even when compact mode is already selected, so Pi reloads cannot leave native rows visible. A single mode change MUST invalidate each affected component no more than once. While its editor wrapper is active, Turn Fold MUST enable Pi TUI's public shrink-clearing behavior so an expanded-to-compact transition clears obsolete screen and scrollback rows. It MUST reassert shrink clearing before compaction because Pi reapplies settings after extension startup during `/reload`, then restore the previous value when it unloads. Turn Fold MUST NOT emit terminal clearing sequences or persist a global Pi setting.
