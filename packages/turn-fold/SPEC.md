...
Turn Fold MUST preserve the native user message.
...
▶ 7 earlier activities · 8 tools · 9 msgs · Ctrl+Shift+O
...
▶ Worked for 14s · 8 tools · 9 msgs · Ctrl+Shift+O
...
▶ Worked for 11s · 1 msg · interrupted · Ctrl+Shift+O
...
Elapsed time comes from persisted turn completion data when available. Time spent between saving and reopening a session MUST NOT increase the displayed duration.
...
`Ctrl+Shift+O` toggles the mode. `Ctrl+O` remains Pi's tool-output expansion control.
