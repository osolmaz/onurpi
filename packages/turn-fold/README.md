...
writing to Pi's session. After a full Pi restart, earlier compactions remain standalone because Pi's
stored compaction entries do not identify their trigger. Historical turns are reconstructed from the
active session branch. Older `live` and `final-only` values are no longer modes and resolve to the
compact default.
