...
import { COLLECTED_OUTPUT_MAX_BYTES } from "../src/collect.ts";
...
import type { ProcessResultDetails } from "../src/tool-result.ts";
import type { ExtensionRuntime } from "../src/tool-types.ts";
...
function requireSessionId(result: ProcessResultDetails): number {
...
describe("runtime integration", () => {
  ...
  it("bounds streamed and killed output while preserving the full log", async () => {
    ...
    const outcome = await terminateSessionById(runtime, sessionId, "SIGTERM");
    assert.ok(outcome);
    // The kill drain stays bounded to the collection window while the exact
    // totals survive for the result envelope.
    assert.ok(outcome.collected.bytes.length <= COLLECTED_OUTPUT_MAX_BYTES);
    assert.equal(outcome.collected.totalBytes, 200000);
