...
export const ExecCommandParameters = Type.Object({
  ...
  cols: Type.Optional(
    Type.Number({
      description: `PTY width in columns (tty: true only; ignored for pipes). Default 120, clamped to ${String(MIN_PTY_COLS)}-${String(MAX_PTY_COLS)}.`,
    }),
  ),
  rows: Type.Optional(
    Type.Number({
      description: `PTY height in rows (tty: true only; ignored for pipes). Default 30, clamped to ${String(MIN_PTY_ROWS)}-${String(MAX_PTY_ROWS)}.`,
    }),
  ),
