/**
 * Bounded, terminal-inert tool results for exec_command / write_stdin /
 * kill_session. Child output is sanitized and tail-capped before it reaches
 * model content or result details; the complete raw stream stays in the
 * per-session log file.
 */

import { randomBytes } from "node:crypto";
import {
  DEFAULT_MAX_BYTES,
  DEFAULT_MAX_LINES,
  formatSize,
  truncateTail,
  type TruncationResult,
} from "@earendil-works/pi-coding-agent";

import type { OnExitPolicy } from "./completion.ts";
import { sanitizeOutputText } from "./output-safety.ts";

const textDecoder = new TextDecoder("utf-8", { fatal: false });

export type ProcessOperation = "exec_command" | "write_stdin";
export type ProcessStatus = "running" | "exited";
export type KillStatus = "killed" | "kill_failed";
export type TruncationMetadata = Omit<TruncationResult, "content">;

function truncationMetadata(result: TruncationResult): TruncationMetadata {
  return {
    truncated: result.truncated,
    truncatedBy: result.truncatedBy,
    totalLines: result.totalLines,
    totalBytes: result.totalBytes,
    outputLines: result.outputLines,
    outputBytes: result.outputBytes,
    lastLinePartial: result.lastLinePartial,
    firstLineExceedsLimit: result.firstLineExceedsLimit,
    maxLines: result.maxLines,
    maxBytes: result.maxBytes,
  };
}

/** Fields shared by every result that carries child-process output. */
export interface OutputResultDetails {
  operation: ProcessOperation | "kill_session";
  status: ProcessStatus | KillStatus;
  running: boolean;
  chunk_id: string;
  wall_time_seconds: number;
  /** Bounded, terminal-inert tail sent to the model and used by the TUI renderer. */
  output: string;
  original_token_count?: number;
  session_id?: number;
  exit_code?: number;
  signal?: string;
  failure_message?: string;
  tty?: boolean;
  log_path?: string;
  cwd?: string;
  command?: string;
  truncation?: TruncationMetadata;
  /** Middle bytes dropped by the in-memory retention caps before result truncation. */
  omitted_bytes?: number;
  /** Cumulative bytes this session has produced since spawn. */
  output_bytes_total?: number;
}

export interface ProcessResultDetails extends OutputResultDetails {
  operation: ProcessOperation;
  status: ProcessStatus;
  yield_time_ms?: number;
  wait_mode?: "relative" | "absolute";
  wait_status?:
    | "completed"
    | "relative_deadline_reached"
    | "absolute_deadline_reached"
    | "cancelled";
  yield_until?: string;
  effective_wait_ms?: number;
  on_exit?: OnExitPolicy;
  completion_notification?: "armed";
  completion_delivery?: "direct";
  on_exit_wake?: "consumed";
  tool_time_utc?: string;
}

export interface KillResultDetails extends OutputResultDetails {
  operation: "kill_session";
  status: KillStatus;
  found: true;
  session_id: number;
  pid?: number;
  requested_signal: string;
  killed: boolean;
  escalated: boolean;
}

/** Long-wait / wake metadata merged into a process result. */
export type ProcessResultExtra = Readonly<{
  wait_mode?: "relative" | "absolute" | undefined;
  wait_status?:
    | "completed"
    | "relative_deadline_reached"
    | "absolute_deadline_reached"
    | "cancelled"
    | undefined;
  yield_until?: string | undefined;
  effective_wait_ms?: number | undefined;
  on_exit?: OnExitPolicy | undefined;
  completion_notification?: "armed" | undefined;
  completion_delivery?: "direct" | undefined;
  on_exit_wake?: "consumed" | undefined;
  tool_time_utc?: string | undefined;
}>;

/**
 * Exact byte/line totals for the complete raw stream, when the caller tracks
 * them (OnurPi's bounded collector does). They keep the truncation marker
 * honest about everything the retention caps dropped, not just the retained
 * payload.
 */
export type ExactTotals = Readonly<{ totalBytes: number; totalLines: number }>;

interface OutputEnvelopeInput {
  wallTimeSec: number;
  collected: Uint8Array;
  logPath?: string | undefined;
  /** Middle bytes dropped by the retention caps during this call's drain. */
  omittedBytes?: number | undefined;
  /** Cumulative bytes the session has produced since spawn. */
  totalBytes?: number | undefined;
  /** Exact totals for the complete raw stream (including omitted middle). */
  exactTotals?: ExactTotals | undefined;
}

export interface FinalizeProcessInput extends OutputEnvelopeInput {
  operation: ProcessOperation;
  sessionId: number | undefined;
  exitCode: number | null | undefined;
  signal: NodeJS.Signals | null;
  failure: string | null;
  tty: boolean;
  cwd?: string | undefined;
  command?: string | undefined;
  yieldTimeMs?: number | undefined;
  /** Long-wait / wake metadata merged into the result (undefined values skipped). */
  extra?: ProcessResultExtra | undefined;
}

export interface FinalizeKillInput extends OutputEnvelopeInput {
  sessionId: number;
  pid?: number | undefined;
  requestedSignal: NodeJS.Signals;
  exitCode: number | null | undefined;
  signal: NodeJS.Signals | null;
  failure: string | null;
  tty: boolean;
  cwd?: string | undefined;
  command?: string | undefined;
  escalated: boolean;
  killed: boolean;
}

function generateChunkId(): string {
  return randomBytes(3).toString("hex");
}

function decode(bytes: Uint8Array): string {
  return textDecoder.decode(bytes);
}

function safeMeta(value: string, max = 4096): string {
  // Metadata sits outside the 50 KiB child-output body. Bound both the scan and
  // rendered value so a malformed path/error cannot recreate an oversized
  // result through a header field.
  const scanLimit = max * 8;
  let end = Math.min(value.length, scanLimit);
  const last = end > 0 ? value.charCodeAt(end - 1) : 0;
  if (end < value.length && last >= 0xd800 && last <= 0xdbff) end--;
  const clipped = end < value.length;
  const clean = sanitizeOutputText(value.slice(0, end)).replace(/\s+/g, " ").trim();
  if (!clipped && clean.length <= max) return clean;
  return `${clean.slice(0, Math.max(0, max - 1))}…`;
}

type OutputEnvelope = Pick<
  OutputResultDetails,
  | "chunk_id"
  | "wall_time_seconds"
  | "output"
  | "original_token_count"
  | "log_path"
  | "truncation"
  | "omitted_bytes"
  | "output_bytes_total"
>;

// eslint-disable-next-line complexity -- Preserve omission semantics for optional envelope fields.
function createOutputEnvelope(input: OutputEnvelopeInput): OutputEnvelope {
  const safeText = sanitizeOutputText(decode(input.collected));
  const retained = truncateTail(safeText, {
    maxBytes: DEFAULT_MAX_BYTES,
    maxLines: DEFAULT_MAX_LINES,
  });
  // Exact totals describe the complete raw stream; retained truncation only
  // sees the sanitized, already-bounded payload.
  const exact = input.exactTotals;
  const wasPreTruncated = exact !== undefined && exact.totalBytes > input.collected.length;
  const truncation: TruncationResult = {
    ...retained,
    truncated: retained.truncated || wasPreTruncated,
    truncatedBy: retained.truncatedBy ?? (wasPreTruncated ? "bytes" : null),
    totalBytes: exact?.totalBytes ?? retained.totalBytes,
    totalLines: exact?.totalLines ?? retained.totalLines,
  };
  const envelope: OutputEnvelope = {
    chunk_id: generateChunkId(),
    wall_time_seconds: input.wallTimeSec,
    output: truncation.content,
    // Mirror codex's rough `approx_token_count` behaviour: 4 bytes ≈ 1 token.
    // Exact totals keep the estimate honest when retention dropped the middle.
    original_token_count: Math.ceil((exact?.totalBytes ?? input.collected.length) / 4),
  };
  if (input.logPath) envelope.log_path = input.logPath;
  if (input.omittedBytes) envelope.omitted_bytes = input.omittedBytes;
  if (input.totalBytes !== undefined) envelope.output_bytes_total = input.totalBytes;
  if (truncation.truncated) {
    envelope.truncation = truncationMetadata(truncation);
  }
  return envelope;
}

// eslint-disable-next-line complexity -- Preserve the stable result field ordering and optional metadata.
export function finalizeProcessResult(input: FinalizeProcessInput): ProcessResultDetails {
  const envelope = createOutputEnvelope(input);
  const running = input.sessionId !== undefined;
  const extra = input.extra;
  return {
    ...envelope,
    operation: input.operation,
    status: running ? "running" : "exited",
    running,
    tty: input.tty,
    ...(input.sessionId === undefined ? {} : { session_id: input.sessionId }),
    ...(input.exitCode === undefined || input.exitCode === null
      ? {}
      : { exit_code: input.exitCode }),
    ...(input.signal ? { signal: input.signal } : {}),
    ...(input.failure ? { failure_message: input.failure } : {}),
    ...(input.cwd ? { cwd: input.cwd } : {}),
    ...(input.command ? { command: input.command } : {}),
    ...(input.yieldTimeMs ? { yield_time_ms: input.yieldTimeMs } : {}),
    ...(extra?.wait_mode ? { wait_mode: extra.wait_mode } : {}),
    ...(extra?.wait_status ? { wait_status: extra.wait_status } : {}),
    ...(extra?.yield_until ? { yield_until: extra.yield_until } : {}),
    ...(extra?.effective_wait_ms !== undefined
      ? { effective_wait_ms: extra.effective_wait_ms }
      : {}),
    ...(extra?.on_exit ? { on_exit: extra.on_exit } : {}),
    ...(extra?.completion_notification
      ? { completion_notification: extra.completion_notification }
      : {}),
    ...(extra?.completion_delivery ? { completion_delivery: extra.completion_delivery } : {}),
    ...(extra?.on_exit_wake ? { on_exit_wake: extra.on_exit_wake } : {}),
    ...(extra?.tool_time_utc ? { tool_time_utc: extra.tool_time_utc } : {}),
  };
}

// eslint-disable-next-line complexity -- Preserve the stable result field ordering and optional metadata.
export function finalizeKillResult(input: FinalizeKillInput): KillResultDetails {
  const envelope = createOutputEnvelope(input);
  return {
    ...envelope,
    operation: "kill_session",
    status: input.killed ? "killed" : "kill_failed",
    running: !input.killed,
    found: true,
    session_id: input.sessionId,
    requested_signal: input.requestedSignal,
    killed: input.killed,
    escalated: input.escalated,
    tty: input.tty,
    ...(input.pid === undefined ? {} : { pid: input.pid }),
    ...(input.exitCode === undefined || input.exitCode === null
      ? {}
      : { exit_code: input.exitCode }),
    ...(input.signal ? { signal: input.signal } : {}),
    ...(input.failure ? { failure_message: input.failure } : {}),
    ...(input.cwd ? { cwd: input.cwd } : {}),
    ...(input.command ? { command: input.command } : {}),
  };
}

/** Pi-bash-style marker appended to model-visible text after a truncated body. */
export function truncationMarker(
  truncation: TruncationMetadata | undefined,
  logPath: string | undefined,
): string | undefined {
  if (!truncation?.truncated) return undefined;
  const full = logPath ? `. Full output: ${safeMeta(logPath)}` : "";
  if (truncation.lastLinePartial) {
    return `[Showing last ${formatSize(truncation.outputBytes)} of final line (line ${String(truncation.totalLines)} is larger than the ${formatSize(DEFAULT_MAX_BYTES)} limit)${full}]`;
  }
  const startLine = truncation.totalLines - truncation.outputLines + 1;
  const endLine = truncation.totalLines;
  if (truncation.truncatedBy === "lines") {
    return `[Showing lines ${String(startLine)}-${String(endLine)} of ${String(endLine)}${full}]`;
  }
  return `[Showing lines ${String(startLine)}-${String(endLine)} of ${String(endLine)} (${formatSize(DEFAULT_MAX_BYTES)} limit)${full}]`;
}

function appendOutputSection(lines: string[], shape: OutputResultDetails): string {
  const header = lines.join("\n");
  const body = shape.output || "(no output)";
  const marker = truncationMarker(shape.truncation, shape.log_path);
  const footer = marker ? `\n\n${marker}` : "";
  return `${header}\n---\n${body}${footer}`;
}

// eslint-disable-next-line complexity -- Preserve the stable response field ordering and optional metadata.
export function renderProcessResultText(shape: ProcessResultDetails): string {
  const lines: string[] = [shape.running ? "[still running]" : "[exited]"];
  if (shape.session_id !== undefined) lines.push(`session_id: ${String(shape.session_id)}`);
  if (shape.exit_code !== undefined) lines.push(`exit_code: ${String(shape.exit_code)}`);
  if (shape.signal) lines.push(`signal: ${safeMeta(shape.signal)}`);
  if (shape.failure_message) lines.push(`failure: ${safeMeta(shape.failure_message)}`);
  if (shape.wait_mode) lines.push(`wait_mode: ${safeMeta(shape.wait_mode)}`);
  if (shape.wait_status) lines.push(`wait_status: ${safeMeta(shape.wait_status)}`);
  if (shape.yield_until) lines.push(`yield_until: ${safeMeta(shape.yield_until)}`);
  if (shape.effective_wait_ms !== undefined)
    lines.push(`effective_wait_ms: ${String(shape.effective_wait_ms)}`);
  if (shape.on_exit) lines.push(`on_exit: ${safeMeta(shape.on_exit)}`);
  if (shape.completion_notification)
    lines.push(`completion_notification: ${safeMeta(shape.completion_notification)}`);
  if (shape.completion_delivery)
    lines.push(`completion_delivery: ${safeMeta(shape.completion_delivery)}`);
  if (shape.on_exit_wake) lines.push(`on_exit_wake: ${safeMeta(shape.on_exit_wake)}`);
  if (shape.tool_time_utc) lines.push(`tool_time_utc: ${safeMeta(shape.tool_time_utc)}`);
  if (shape.log_path) lines.push(`log_path: ${safeMeta(shape.log_path)}`);
  if (shape.cwd) lines.push(`cwd: ${safeMeta(shape.cwd)}`);
  lines.push(`wall_time_seconds: ${shape.wall_time_seconds.toFixed(3)}`);
  lines.push(`chunk_id: ${shape.chunk_id}`);
  if (shape.original_token_count !== undefined)
    lines.push(`original_token_count: ${String(shape.original_token_count)}`);
  if (shape.output_bytes_total !== undefined)
    lines.push(`output_bytes_total: ${String(shape.output_bytes_total)}`);
  if (shape.omitted_bytes) lines.push(`omitted_bytes: ${String(shape.omitted_bytes)}`);
  lines.push(`tty: ${String(shape.tty)}`);
  return appendOutputSection(lines, shape);
}

// eslint-disable-next-line complexity -- Preserve the stable response field ordering and optional metadata.
export function renderKillResultText(shape: KillResultDetails): string {
  const lines: string[] = [shape.killed ? "[killed]" : "[kill failed]"];
  lines.push(`session_id: ${String(shape.session_id)}`);
  if (shape.pid !== undefined) lines.push(`pid: ${String(shape.pid)}`);
  lines.push(`requested_signal: ${safeMeta(shape.requested_signal)}`);
  lines.push(`killed: ${String(shape.killed)}`);
  lines.push(`running: ${String(shape.running)}`);
  lines.push(`escalated: ${String(shape.escalated)}`);
  if (shape.exit_code !== undefined) lines.push(`exit_code: ${String(shape.exit_code)}`);
  if (shape.signal) lines.push(`signal: ${safeMeta(shape.signal)}`);
  if (shape.failure_message) lines.push(`failure: ${safeMeta(shape.failure_message)}`);
  if (shape.log_path) lines.push(`log_path: ${safeMeta(shape.log_path)}`);
  if (shape.cwd) lines.push(`cwd: ${safeMeta(shape.cwd)}`);
  lines.push(`wall_time_seconds: ${shape.wall_time_seconds.toFixed(3)}`);
  lines.push(`chunk_id: ${shape.chunk_id}`);
  if (shape.original_token_count !== undefined)
    lines.push(`original_token_count: ${String(shape.original_token_count)}`);
  if (shape.output_bytes_total !== undefined)
    lines.push(`output_bytes_total: ${String(shape.output_bytes_total)}`);
  if (shape.omitted_bytes) lines.push(`omitted_bytes: ${String(shape.omitted_bytes)}`);
  lines.push(`tty: ${String(shape.tty)}`);
  return appendOutputSection(lines, shape);
}
