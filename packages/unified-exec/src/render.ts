...
// eslint-disable-next-line complexity -- Audited upstream bounded-scan sanitizer; keep the clipping branches together.
...
export function renderExecCommandCall(
  ...
  markStarted(context.state, context.executionStarted);
...
export function renderSetOnExitCall(
  ...
  const banner =
    ...
    theme.fg("muted", ` session_id=${sid} → ${policy}`);
  const text = context.lastComponent instanceof Text ? context.lastComponent : new Text("", 0, 0);
  text.setText(banner);
  return text;
}
...
export function renderKillSessionCall(
  ...
  const sid = args?.session_id !== undefined ? args.session_id : "?";
  const signal =
    typeof args?.signal === "string" ? safeOneLine(args.signal) || "SIGTERM" : "SIGTERM";
  const banner =
...
export function renderListSessionsCall(
  _args: object,
  theme: Theme,
  context: UnifiedRenderContext<object>,
): Component {
  const text = context.lastComponent instanceof Text ? context.lastComponent : new Text("", 0, 0);
...
/** A minimal Container-like wrapper so the result view can rebuild per tick. */
class OutputResultContainer extends Container {
  state: RenderState = {};
}

// eslint-disable-next-line complexity -- Preserve the upstream ticker and container lifecycle branches.
function renderOutputResult(
  result: AgentToolResult<UnifiedExecDetails>,
  options: ToolRenderResultOptions,
  theme: Theme,
  context: UnifiedRenderContext<unknown>,
): Component {
  ...
  const container =
    context.lastComponent instanceof OutputResultContainer
      ? context.lastComponent
      : new OutputResultContainer();
...
/**
 * Invalidate the cached collapsed preview when the streamed body changes.
 * Width alone is not a sufficient cache key: partial updates reuse the same
 * context state and component while output grows.
 */
...
function rebuildOutputResult(
  ...
  // Prefer the structured `details.output` (bounded, sanitized body). Fall
  // back to the result's content text (which during streaming is the raw
  // tail); details.output is always populated, so this fallback is rare.
  ...
  if (truncation?.truncated) {
    ...
    if (truncation.lastLinePartial) {
      message = `Truncated: last ${formatSize(truncation.outputBytes)} of line ${String(truncation.totalLines)} shown (${formatSize(DEFAULT_MAX_BYTES)} limit)${logInfo}`;
    } else if (truncation.truncatedBy === "lines") {
      message = `Truncated: showing ${String(truncation.outputLines)} of ${String(truncation.totalLines)} lines${logInfo}`;
    } else {
      message = `Truncated: ${String(truncation.outputLines)} lines shown (${formatSize(DEFAULT_MAX_BYTES)} limit)${logInfo}`;
    }
...
function buildStatusLine(
  ...
  if (details.operation === "kill_session") {
    bits.push(
      details.killed
        ? theme.fg("success", `killed session_id=${String(details.session_id ?? "?")}`)
        : theme.fg(
            "error",
            `FAILED session_id=${String(details.session_id ?? "?")} · still running`,
          ),
    );
    if (details.pid !== undefined) bits.push(`pid=${String(details.pid)}`);
    ...
    if (details.exit_code !== undefined && details.exit_code !== null) {
      bits.push(`exit_code=${String(details.exit_code)}`);
    } else if (details.signal) {
      bits.push(`signal=${safeOneLine(details.signal)}`);
    }
  } else if (details.running && details.session_id !== undefined) {
    bits.push(`session_id=${String(details.session_id)}`);
  } else if (details.exit_code !== undefined && details.exit_code !== null) {
    bits.push(
      details.exit_code === 0
        ? `exit_code=${String(details.exit_code)}`
        : theme.fg("error", `exit_code=${String(details.exit_code)}`),
    );
  } else if (details.signal) {
  ...
  if (details.failure_message)
    bits.push(theme.fg("error", `failure: ${safeOneLine(details.failure_message)}`));
...
// eslint-disable-next-line complexity -- Preserve the upstream session-inventory rendering branches.
export function renderListSessionsResult(
  ...
  const container =
    context.lastComponent instanceof ListResultContainer
      ? context.lastComponent
      : new ListResultContainer();
  container.clear();
  const sessions = details.sessions;
  const active = details.active_count ?? sessions.filter((session) => session.running).length;
  const justExited =
    details.just_exited_count ?? sessions.filter((session) => !session.running).length;
  const summary = `${String(active)} live${justExited ? ` · ${String(justExited)} just exited` : ""}`;
    ...
    container.addChild({
      render: (width: number): string[] => {
        ...
        for (const session of shown) {
          lines.push(truncateToWidth(renderSessionLine(session, theme), width, "..."));
          if (options.expanded) {
            lines.push(
              truncateToWidth(
                theme.fg("muted", `  log: ${safeOneLine(tildify(session.log_path))}`),
                width,
                "...",
              ),
            );
        ...
        if (!options.expanded && sessions.length > shown.length) {
          ...
          const hint =
            theme.fg("muted", `... (${String(hidden)} more sessions,`) +
            ` ${keyHint("app.tools.expand", "to expand")}${theme.fg("muted", ")")}`;
  ...
  if (details.tool_time_utc) {
    container.addChild(
      new Text(
        `\n${theme.fg("muted", `tool_time_utc: ${safeOneLine(details.tool_time_utc)}`)}`,
        0,
        0,
      ),
    );
  ...
  const state = session.running
    ...
    : theme.fg(
        ...
        session.exit_code !== undefined && session.exit_code !== null
          ? `exited=${String(session.exit_code)}`
